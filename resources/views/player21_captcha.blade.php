...
<head>
    ...
    <title>player</title>
    <link rel="stylesheet" href="/player/css/player.css?v={{ hash_file('md5', public_path('player/css/player.css')) }}">
...
<div id="globalFx" class="gfx letterbox on"><i></i></div>
...
<script>
    ...
    cdn.player = (function () {
        ...
        window.__playbackConfigReady = false;
        getTurnstileTokenForPlayback(function (captchaToken) {
            const data = fetchPlaybackUrls(captchaToken);
            CDNplayerConfig.file = data.file_string || '';
            window.__playbackConfigReady = true;
            fxOff();
        });
    ...
    $(function () {
        ...
        if (window.__playbackConfigReady) {
            cdn.player.buildCDNplayer();
        } else {
            const waitPlaybackConfig = setInterval(function () {
                if (window.__playbackConfigReady) {
                    clearInterval(waitPlaybackConfig);
                    cdn.player.buildCDNplayer();
                }
            }, 50);
        }
...
<noscript>
    <div><img src="https://mc.yandex.ru/watch/104007594" style="position:absolute; left:-9999px;" alt=""/></div>
</noscript>
<!-- /Yandex.Metrika counter -->
