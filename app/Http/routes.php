...
// api общего доступа
Route::match(['get','post'],'/api/actors', ['middleware'=>[], 'uses'=>'PublicApiController@actors']);
Route::match(['get','post'],'/api/directors', ['middleware'=>[], 'uses'=>'PublicApiController@directors']);
...
// Route::post('/show/{type}/{id}', ['middleware'=>['showMiddleware'], 'uses'=>'ShowController@player'])->where('id', 'kinopoisk|imdb')->where('id', '[a-z0-9]+');
Route::get('/share/{id}', ['uses'=>'ShowController@share'])->where('id', '[0-9]+');
Route::get('/tgapp', ['uses'=>'ShowController@tgApp']);
Route::post('/download/{id}', ['middleware' => [], 'uses'=>'ShowController@download'])->where('id', '[0-9]+');
// для васта
Route::get('/share/{id}', ['uses'=>'ShowController@share'])->where('id', '[0-9]+');
...
// Debug
Route::get('/host/HOQDotsh1MEnIl', function () {
    echo $_SERVER['DOCKER_HOSTNAME'];
});
